...
Displays translated text seamlessly without cluttered headers or buttons.
...
- `MAX_WIDTH = 440`, `MIN_WIDTH = 260`
- `MAX_HEIGHT = 280`, `MIN_HEIGHT = 70`
- `BORDER_RADIUS = 12`
...
- `_setup_ui()`: Creates root layout, read-only `QTextEdit` and subtle hint label.
- `_setup_shortcuts()`: Sets application-wide shortcuts for `Ctrl+C`, `Ctrl+X`, and `Escape`.
- `show_translation(text: str, pos: QPoint = None)`: Formats text, dynamically computes width/height, positions near cursor, and starts the 50 FPS cursor-tracking timer.
- `_copy_and_close()`: Copies text to clipboard, turns border into vibrant neon green (`#30D158`), sets hint to *"✓ Copied to clipboard!"*, and fades out after a satisfying 300ms feedback delay.
